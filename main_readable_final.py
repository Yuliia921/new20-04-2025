...
async def generate_pdf(
    ...
    # Заключение и рекомендации — отдельным блоком
    pdf.ln(4)
    pdf.set_font("DejaVu", "B", 12)
    pdf.multi_cell(0, 8, "Заключение:")
    pdf.set_font("DejaVu", "", 12)
    pdf.multi_cell(0, 8, clean(conclusion))
    pdf.ln(2)

    pdf.set_font("DejaVu", "B", 12)
    pdf.multi_cell(0, 8, "Рекомендации:")
    pdf.set_font("DejaVu", "", 12)
    pdf.multi_cell(0, 8, clean(recommendations))

    # Подпись и контакты
    pdf.ln(10)
    pdf.set_font("DejaVu", "", 11)
    pdf.multi_cell(0, 8, "врач акушер-гинеколог Куриленко Юлия Сергеевна")
    pdf.multi_cell(0, 8, "Телефон: +37455987715")
    pdf.multi_cell(0, 8, "Telegram: https://t.me/doc_Kurilenko")
