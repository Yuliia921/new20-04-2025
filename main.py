...
async def generate_pdf(
    ...
    for label, value in fields:
        ...
        text_line = f"{safe_label}: {safe_value}"
        if len(text_line.strip()) < 3:
            text_line = f"{safe_label}: -"
        try:
            pdf.multi_cell(180, 10, text_line)
        except:
            pdf.multi_cell(180, 10, f"{safe_label}: -")
        pdf.ln(1)

    pdf.ln(5)
    pdf.set_font("DejaVu", "", 11)
    pdf.multi_cell(180, 8, "врач акушер-гинеколог Куриленко Юлия Сергеевна")
    pdf.multi_cell(180, 8, "Телефон: +37455987715")
    pdf.multi_cell(180, 8, "Telegram: https://t.me/doc_Kurilenko")
