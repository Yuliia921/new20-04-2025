...
async def search(request: Request, query: str = ""):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("SELECT id, date, template FROM protocols WHERE fio LIKE ?", ('%' + query + '%',))
...
async def view_protocol(request: Request, protocol_id: int):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("SELECT content FROM protocols WHERE id = ?", (protocol_id,))
    row = cursor.fetchone()
    conn.close()
    if row:
        content = row[0].replace("\n", "<br>")
        return HTMLResponse(content=f"<h3>Просмотр протокола</h3><p>{content}</p><a href='/search'>← Назад</a>")
    return HTMLResponse(content="Протокол не найден", status_code=404)

@app.get("/download/{protocol_id}")
async def download_protocol(protocol_id: int):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("SELECT fio, date, template, content FROM protocols WHERE id = ?", (protocol_id,))
    row = cursor.fetchone()
    conn.close()
    ...
    pdf.multi_cell(180, 10, f"{template}\nДата: {date}\nФИО: {fio}\n\n{content}")
    pdf.ln(10)
    pdf.set_font("DejaVu", "", 9)
    pdf.cell(0, 6, "врач акушер-гинеколог Куриленко Юлия Сергеевна", ln=True)
    pdf.cell(0, 6, "Телефон: +37455987715", ln=True)
    ...
    filename = f"/tmp/protocol_{protocol_id}.pdf"
    pdf.output(filename)
    return FileResponse(filename, media_type="application/pdf", filename=f"protocol_{protocol_id}.pdf")

@app.get("/delete/{protocol_id}")
async def delete_protocol(protocol_id: int):
    ...
    conn.commit()
    conn.close()
    return HTMLResponse(content="<p>Протокол удалён</p><a href='/search'>← Назад</a>")
